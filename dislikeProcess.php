...
if (isset($_SESSION['user_id']) && isset($_POST['commentID'])) {
    ...
    $query = "SELECT * FROM dislike_comment WHERE u_id = '$userID' AND c_id = '$commentID'";
    $result = mysqli_query($con, $query);

    if (mysqli_num_rows($result) > 0) {
        $query = "DELETE FROM dislike_comment WHERE u_id = '$userID' AND c_id = '$commentID'";
        $result = mysqli_query($con, $query);
        http_response_code(410); //user already disliked the comment
        exit();
    } else {
        $query = "DELETE FROM like_comment WHERE u_id = '$userID' AND c_id = '$commentID'";
        $result = mysqli_query($con, $query);
        $query = "INSERT INTO dislike_comment (u_id, c_id) VALUES ('$userID', '$commentID')";
        $result = mysqli_query($con, $query);
