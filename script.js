...
//   if (like.querySelector("path").getAttribute("d") === originalPath) {
...
//   if (dislike.querySelector("path").getAttribute("d") === originalPath) {
    ...
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        document.getElementById("like-" + commentID).innerHTML =
          xhr.responseText;
        document.getElementById("like-count-" + commentID).value += 1;
